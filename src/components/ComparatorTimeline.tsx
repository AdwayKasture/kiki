import { User, Wrench, MessageSquare, Sparkles, Eye, EyeOff } from 'lucide-react'
...
interface ComparatorTimelineProps {
  entries: Entry[]
...
export function ComparatorTimeline({
  entries,
  highlightedIds,
  ignoredIds,
  rowRef,
      ...
      {entries.map((entry, index) => {
        ...
        return (
          ...
          >
              ...
              {entry.type === 'tool' ? (
                ...
                <p
                  className={cn(
                    ...
                    isHighlighted && !isIgnored ? 'text-[var(--text-h)]' : 'text-[var(--text)]',
            ...
            {isTool && onToggleIgnore && (
              <button
                type="button"
                onClick={() => onToggleIgnore(entry.id)}
                className={cn(
                  'ml-2 shrink-0 rounded p-1.5 opacity-0 transition-opacity group-hover:opacity-100',
                  isIgnored
                    ? 'text-[var(--accent)] hover:bg-[var(--accent-bg)]'
                    : 'text-[var(--text)] hover:bg-[var(--surface)]',
                )}
                title={isIgnored ? 'Include in matching' : 'Ignore in matching'}
              >
                {isIgnored ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
              </button>
            )}
          </div>
        )
